...
2. The defaults are local `faster-whisper medium.en` transcription (CPU INT8) and Alibaba Cloud `cosyvoice-v3.5-flash` TTS with an automatically enrolled per-video source voice. Require a Beijing-region `DASHSCOPE_API_KEY`, plus `YOUTUBE_DUB_VOICE_SAMPLE_BASE_URL` (or `--voice-sample-base-url`) for an authorized short-lived public sample endpoint. The launcher safely loads the DashScope key, workspace, optional fixed voice, and sample base URL from `.secrets/dashscope.env` in the Skill directory or its parent. Use `DASHSCOPE_WORKSPACE_ID` for the preferred workspace endpoint; the compatible shared Beijing endpoint is used when it is absent. Use `--aliyun-voice-mode fixed` with `ALIYUN_COSYVOICE_VOICE` or `--voice` only when the user explicitly requests a fixed voice. Require `OPENROUTER_API_KEY` only when `--tts-backend mai` or `--transcriber-backend openrouter-whisper1` is selected. Never expose, recover, log, or place API keys on the command line. English polishing, topic detection, Chinese translation, and timing rewrites are completed by the Agent currently running this Skill, using its own configured backend model; they do not invoke a separate model CLI.
3. Run with the project's virtual-environment Python. Do not invoke the launcher with Hermes/global `python3`; changing `PATH` after interpreter startup does not make `.venv` packages such as `faster-whisper` visible.

   ```bash
   "$SKILL_DIR/.venv/bin/python" "$SKILL_DIR/scripts/run_youtube_zh_dub.py" 'YOUTUBE_URL'
   ```
...
- Per-video hosted source-voice synthesis: extract a 20-second sample around the longest English speech segment, publish it through the authorized short-lived endpoint, create and poll an Alibaba `voice-enrollment` voice until `OK`, revoke the sample, and reuse the registered voice on resume. Synthesize each Chinese sentence with `cosyvoice-v3.5-flash`, download its expiring result URL immediately, and cache the local 24 kHz WAV by text, model, voice, speed, and instruction.
...
When Alibaba Cloud voice enrollment uses this host's authorized Nginx endpoint, the main pipeline publishes only the final 16-bit WAV, uses the returned URL immediately, revokes it after enrollment succeeds or fails, and stores only the resulting `voice_id`, source fingerprint, model, and status in `segments/aliyun_voice_enrollment.json`. It never stores the signed URL or returned `resource_link`. Never publish samples under the general `/home/hermes/data` download root. The IP-only endpoint is unencrypted HTTP; disclose that limitation and prefer private OSS or trusted HTTPS when available.
